...
public class GeneralPlanner implements Planner {
     ...
     * 
     * @param selClause
     *            an object describing the query to be performed
     * 
     * @return a plan tree for executing the specified query
     * 
     * @throws java.io.IOException
     *             if an IO error occurs when the planner attempts to load
     *             schema and indexing information.
     */
    @Override
    ...
    private PlanNode makeGeneralSelect(SelectClause selClause,
        ...
        if (!orderExpressions.isEmpty()) {
            planNode = (PlanNode) new SortNode((PlanNode) projNode,
                    orderExpressions);
        }
    ...
    private PlanNode makeJoinExpression(FromClause joinClause)
            throws IOException {
        ...
        if (fromLeft.isBaseTable()) {
            leftNode = makeSimpleSelect(fromLeft.getTableName(), null, null);
        } else {
            leftNode = makeGeneralSelect(fromLeft.getSelectClause(), null);
        }

        if (fromRight.isBaseTable()) {
            rightNode = makeSimpleSelect(fromRight.getTableName(), null, null);
        } else {
     ...
     * 
     * @param tableName
     *            The name of the table that is being selected from.
     * 
     * @param predicate
     *            An optional selection predicate, or {@code null} if no
     *            filtering is desired.
     * 
     * @return A new plan-node for evaluating the select operation.
     * 
     * @throws IOException
     *             if an error occurs when loading necessary table information.
     */
    public SelectNode makeSimpleSelect(String tableName, Expression predicate,
        ...
        // Open the table.
        TableInfo tableInfo = storageManager.getTableManager().openTable(
                tableName);
